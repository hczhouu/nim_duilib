...
void ShowMessageBox(HWND hwnd, MsgboxCallback cb, const std::wstring& strText,
	const std::wstring& strCaption, const std::wstring& strYes, 
	const std::wstring& strNo, const MsgBoxType& mbType)
{
	MsgBox* pMsgbox = new MsgBox;
	HWND hWnd = pMsgbox->Create(hwnd, L"", WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX, 0);
	if (hWnd == NULL)
		return;
	MutiLanSupport* multilan = MutiLanSupport::GetInstance();
	pMsgbox->SetTitle(strCaption);
	pMsgbox->SetContent(strText);
	pMsgbox->SetMsgBoxType(strYes, strNo, mbType);
	pMsgbox->Show(hwnd, cb);
}
...
void MsgBox::SetMsgBoxType(const std::wstring& strYes, const std::wstring& strNo, const MsgBoxType& mbType)
{
	btn_yes_->SetText(strYes);
	btn_no_->SetText(strNo);
	btn_yes_->SetVisible(true);
	btn_yes_->SetFocus();
	if (mbType == MB_INFORMATION)
	{
		btn_no_->SetVisible(false);
	}
	else if (mbType == MB_QUESTION)
	{
		btn_no_->SetVisible(true);
	}
	else if (mbType == MB_ERROR)
	{
		btn_no_->SetVisible(false);
	}
}
